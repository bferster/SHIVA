...
{var i;var dd=container+"Stp";$(con).height(40);var str="<span id='"+dd+"'>";for(i=0;i<items.length;++i){str+="<input type='radio' id='stp"+i+"' name='stepper'";if(!i)
...
{var hgt=40,wid=40;var dd=con+"Int";$(dd).remove();$(con).append("<div id='"+dd.substr(1)+"'/>");options.orientation=options.orientation.toLowerCase();options.step-=0;options.max-=0;options.min-=0;if(options.orientation=="vertical"){$(dd).css("height","100%");hgt=$(con).height();if(shivaLib.options.height)
hgt=shivaLib.options.height;}
else{$(dd).css("width","100%");wid=$(con).width();if(shivaLib.options.width)
wid=shivaLib.options.width;}
$(con).width(wid);$(con).height(hgt);if(options.type=="Bar")
...
shivaLib.SendShivaMessage("ShivaSlider=click",(which+1)+"|"+val);});$(dd).bind("slide",function(e,ui){var which=-1;var val=ui.value;if(ui.values)
val=ui.values[0];if(ui.value!=val){which=0;val=ui.values[1];}
shivaLib.SendShivaMessage("ShivaSlider=move",(which+1)+"|"+val);});DrawSliderTicks();}
...
hgt=$(con).height();else
wid=$(con).width();$('#sliderBack').attr('width',wid);$('#sliderBack').attr('height',hgt);var inc=Number(Math.max(hgt,wid)/(n+1));var pos=inc;var tinc=Math.abs(max-min)/(n+1);var tpos=tinc;sliderContext.fillStyle=options.textCol;if(!isVert)
...
sliderContext.fillText(val,18,Number(hgt)-pos+3);else
...
{var i,o,nChars=0;var dd=container+"Sel";var str="<span id='"+dd+"'>";$(con).height(40);for(i=0;i<items.length;++i){o=items[i];nChars+=o.label.length+5;if(items[i].type)nChars+=4;if(options.style=="Button")
...
shivaLib.SendShivaMessage("ShivaSelect=click",id+"|"+ch)});else
...
{var o,i,v,sty,str="";var dd="#"+container+"Dlg";$(dd).remove();$(con).append("<div id='"+container+"Dlg' style='width:auto;border:1px solid #999;padding:8px;background-color:#f8f8f8;text-align:left' class='rounded-corners'/>");for(o in options){v=options[o];if(v=="true")v=true;if(v=="false")v=false;options[o]=v;}
...
shivaLib.SendShivaMessage("ShivaDialog=click",id+"|checked")});else if(sty=="checkbox")
$("#"+o.name).click(function(){var id=this.id.substr(5)
var ch=this.checked?"checked":"unchecked"
shivaLib.SendShivaMessage("ShivaDialog=click",id+"|"+ch)});else if((sty=="input")||(sty=="combo"))
$("#"+o.name).change(function(){var id=this.id.substr(5)
var ch=this.value;shivaLib.SendShivaMessage("ShivaDialog=click",id+"|"+ch)});else if(sty=="range"){if(o.def=="false")o.def=0;var ops={min:0,max:100,value:o.def-0,stop:function(event,ui){var id=this.id.substr(5)
shivaLib.SendShivaMessage("ShivaDialog=click",id+"|"+ui.value)}};$("#"+o.name).slider(ops);}}}
...
$(dd).append(str);$("#Clo"+dd.substr(1)).click(function(){var id=this.id.substr(3);$("#"+id).hide();shivaLib.SendShivaMessage("ShivaDialog=close",options.title);});}}}
...
time=shivaLib.TimecodeToSeconds(time);}
shivaLib.player.play(time);}
...
{var v=msg.split("|");if(v[0]=="ShivaAct=resize"){if(v[1]=="100"){$("#containerDiv").width("100%");$("#containerDiv").height("100%");}}
else if(v[0]=="ShivaAct=play"){this.VideoPlay();if(v[1]!=undefined)
this.Videoplay(v[1]);}
else if(v[0]=="ShivaAct=pause")
this.VideoPause();else if(v[0]=="ShivaAct=load"){this.VideoLoad(v[1]);}}
...
{var str;$("#shivaNotesDiv").remove();if(this.options.notes!="true")
return;var con=$("#"+this.container);str="<div id='shivaNotesDiv' style='position:absolute;padding:8px;overflow-y:auto;";str+="width:500px;height:"+(con.height()-16)+"px;";str+="background-color:#f8f8f8;border:1px solid #ccc;box-shadow:4px 4px 8px #ccc;";var top=con.offset().top;var left=con.offset().left+con.width()+16;str+="top:"+top+"px;left:"+left+"px;'>";str+="<table id='shivaNotesTbl' width='100%'>";var ts="color:#009900;cursor:pointer";var ns="font-size:small;border:none;background:none;width:100%;padding:0px;margin:0px";str+="<div style='text-align:center;font-size:large;'><img src='shivalogo16.png' style='vertical-align:-2px'><b> SHIVA Notes</b></div><hr>";str+="<div style='position:absolute;top:-2px;left:0px;width:100%;text-align:right'><br/>Find: <input type='input' id='shivaNotesSearch' style='height:12px;width:60px;font-size:x-small;padding:0px;margin:0px'/>&nbsp; &nbsp;</div>"
str+="<tr><td width='38' id='ntc-0' style='"+ts+"'>Type:</td><td><input id='ntx-0' type='input' style='"+ns+"'/></td></tr>";str+="</table>";str+="<div style='text-align:right'><br/>________________________________<br/><br/>Pause video while typing?<input type='checkbox' id='notesPause' style='height:11px'>";str+="<br/>Save notes: <img src='savedot.gif' id='shivaNotesSave' title='Save notes' width='15' style='vertical-align:bottom'>";$('body').append(str+"</div>");$("#shivaNotesDiv").draggable();$("#ntx-0").focus();$("#shivaNotesSearch").on("keydown",function(e){var n=$("#shivaNotesTbl tr").length;var patt=new RegExp($("#shivaNotesSearch").val());for(var i=0;i<n;++i){$("#ntx-"+i).css("color","black");if(($("#ntx-"+i).val())&&($("#ntx-"+i).val().match(patt)))
...
else if(!$("#ntx-"+rowNum).val()){$("#ntc-"+rowNum).text(shivaLib.SecondsToTimecode(shivaLib.VideoTime()));if($("#notesPause").prop('checked'))
shivaLib.VideoPause();$("#ntc-"+rowNum).click(function(e){var time=$("#"+e.target.id).text();if(e.shiftKey)
$("#"+e.target.id).text(shivaLib.SecondsToTimecode(shivaLib.VideoTime()));else
shivaLib.VideoTime(shivaLib.TimecodeToSeconds(time));});$("#ntc-"+rowNum).dblclick(function(e){var time=$("#"+e.target.id).text();shivaLib.VideoPlay(time);});}});};SHIVA_Show.prototype.DrawSubway=function(oldItems)
...
{var i,j,o,shape,id=0;var options=this.options;var con="#"+this.container;var w=options.width;var h=options.height;var svg=null,nodes=null,edges=null,labels=null;var dataSet=null;var d3Zoom;var minZoom=.1,maxZoom=10;var margins=[0,0,0,0];var canPan=true;var firstTime=true;var unselectable={"-moz-user-select":"none","-khtml-user-select":"none","-webkit-user-select":"none","-ms-user-select":"none","user-select":"none","pointer-events":"none"}
...
$(con).height(options.width),h=w;$(con).html("");var colors=d3.scale.category10();if((options.chartType=="Network")||(options.chartType=="Chord")){if(options.dataSourceUrl)
...
this.GetSpreadsheet(options.dataSourceUrl,false,null,function(data){dataSet=[];var nRows=data.length;var nSets=data[0].length-1;for(i=1;i<nSets;++i){for(j=1;j<nRows;++j){o={};o.key=data[0][i];o.date=new Date(data[j][0]);o.value=data[j][i]-0;dataSet.push(o);}}
redraw();},true);}
...
return styles[d.style].alpha;}).on("click",function(d){if(!d3.event.shiftKey)AddPopup(d);}).call(force.drag);nodes.append("title").text(function(d){var str=d.info;if(d.info.match(/http/)){var v=(str+" ").match(/http.?:\/\/.*?\s/ig);for(var i=0;i<v.length;++i){v[i]=v[i].trim();str=str.replace(RegExp(v[i].replace(/[-[\]{}()*+?.,\\^$|#\s]/g,"\\$&")),"");}}
...
else if(options.chartType=="Tree"){margins=[20,20,20,options.spacing/2];if(firstTime)
...
return"translate("+dataSet.y0+","+dataSet.x0+")";}).on("click",function(d){toggle(d);redraw(d);});nodeEnter.append("circle").attr("r",1e-6).style("stroke","#999").style("cursor",function(d){return d._children?"pointer":"auto";}).style("fill",function(d){return d._children?"#"+options.nCol:"#fff";});nodeEnter.append("text").style("font-family","sans-serif").attr("x",function(d){var dx=options.lSize/1.5;return d.children||d._children?-dx:dx;}).attr("dy",".3em").attr("text-anchor",function(d){return d.children||d._children?"end":"start";}).text(function(d){return d.name;}).style("fill-opacity",1e-6).style("fill",options.lCol).style("font-size",options.lSize).style(unselectable);var nodeUpdate=node.transition().duration(options.trans).attr("transform",function(d){return"translate("+d.y+","+d.x+")";});nodeUpdate.select("circle").attr("r",options.lSize/2.67).style("fill",function(d){return d._children?"#"+options.nCol:"#fff";});nodeUpdate.select("text").style("fill-opacity",1);var nodeExit=node.exit().transition().duration(options.trans).attr("transform",function(d){return"translate("+d.parent.y+","+d.parent.x+")";}).remove();nodeExit.select("circle").attr("r",1e-6);nodeExit.select("text").style("fill-opacity",1e-6);var link=svg.selectAll("path").data(tree.links(nodes),function(d){return d.target.id;});link.enter().insert("path","g").style("fill","none").style("stroke","#"+options.eCol).style("stroke-width",options.eWid).attr("d",function(d){var o={x:d.source.x,y:d.source.y};return diagonal({source:o,target:o});})
...
else if(options.chartType=="Bubble"){colors=d3.scale.category20c();var dia=Math.min(options.height,options.width)-8;if(options.style=="Packed"){var pack=d3.layout.pack().size([dia,dia]).value(function(d){return d.val?d.val:1});node=svg.datum(dataSet).selectAll(".node").data(pack.nodes).enter().append("g").attr("transform",function(d){return"translate("+d.x+","+d.y+")";});node.append("title").text(function(d){var str=d.name;if(!d.children&&d.val)
str+=": "+d.val;return str});node.append("circle").attr("r",function(d){return d.r;}).style("stroke","#"+options.gCol).style("fill",function(d){return d.children?"#"+options.gCol:"#"+options.nCol;}).style("fill-opacity",function(d){return d.children?.15:1}).style("cursor",function(d){return d.info?"pointer":"auto";}).on("click",AddPopup)
node.filter(function(d){return!d.children;}).append("text").attr("dy",".3em").attr("font-family","sans-serif").attr("text-anchor","middle").attr("font-size",options.lSize+"px").attr("fill","#"+options.lCol).style("text-anchor","middle").style(unselectable).text(function(d){return d.name.substring(0,d.r/3);});}
...
var colorSet=d3.scale.ordinal().range(colorRange);var x=d3.time.scale().range([0,options.width]);var y=d3.scale.linear().range([options.height-options.lSize-10,options.lSize*1+10]);var timeBar=d3.select(con).append("div").style("position","absolute").style("top",(options.height-options.lSize-6)+"px").style("left","0px").style("font-size",options.lSize+"px").style("color","#"+options.lCol).style("font-family","sans-serif").html("<span id='startDate'></span><span id='endDate' style='position:absolute;left:"+(options.width-200)+"px;width:200px;text-align:right'></span>")
var dataBar=d3.select(con).append("div").style("position","absolute").style("width","2px").style("height",options.height-options.lSize*2-20+"px").style("pointer-events","none").style("top",(options.lSize-0+10)+"px").style("left","0px").style("background","#fff").style("font-size",options.lSize+"px").style("color","#"+options.lCol).style("font-family","sans-serif").html("<div id='vdat' style='background-color:#"+options.backCol+";position:absolute;left:-100px;top:"+(-options.lSize-6)+"px;width:200px;text-align:center'></div><div id='vnow' style='background-color:#"+options.backCol+";position:absolute;left:-100px;top:"+(options.height-options.lSize*2-16)+"px;width:200px;text-align:center'></div>")
...
svg.selectAll(".layer").data(layers).enter().append("path").attr("class","layer").attr("d",function(d){return area(d.values);}).style("fill",function(d,i){return colorSet(i);});svg.selectAll(".layer").attr("opacity",1).on("mouseover",function(d,i){svg.selectAll(".layer").transition().duration(250).attr("opacity",function(d,j){return j!=i?0.6:1;})}).on("mousemove",function(d,i){var k,o,datearray=[];var date=x.invert(d3.mouse(this)[0]);var now=date.getFullYear()*3650+date.getMonth()*300+date.getDate();var selected=(d.values);for(var k=0;k<selected.length;k++){o=selected[k].date;datearray[k]=o.getFullYear()*3650+o.getMonth()*300+o.getDate();}
k=datearray.indexOf(now);d3.select(this).attr("stroke","#000").attr("stroke-width","0.5px")
...
dataBar.style("visibility","visible");}).on("mouseout",function(d,i){svg.selectAll(".layer").transition().duration(250).attr("opacity","1");d3.select(this).attr("stroke-width","0px");dataBar.style("visibility","hidden");})
$("#startDate").text(shivaLib.FormatDate(x.invert(0),options.dateFormat));$("#endDate").text(shivaLib.FormatDate(x.invert(options.width),options.dateFormat));}
else if(options.chartType=="Parallel"){var y={};var dragging={};canPan=false;var x=d3.scale.ordinal().rangePoints([0,options.width],1);var line=d3.svg.line();var axis=d3.svg.axis().orient("left").ticks(4).outerTickSize(0);svg.attr("height",options.height-options.lSize*2+"px");svg.attr("transform","translate(0,"+options.lSize*3+")");x.domain(dimensions=d3.keys(dataSet[0]).filter(function(d){return d!="name"&&(y[d]=d3.scale.linear().domain(d3.extent(dataSet,function(p){return+p[d];})).range([options.height-options.lSize*4,0]));}));function position(d){var v=dragging[d];return v==null?x(d):v;}
...
else if(options.chartType=="Chord"){canPan=false;options.height=options.width;var outerRadius=options.width/2;var innerRadius=outerRadius-options.padding;var cols=[];if(options.sCol!="none"){var v=options.sCol.split(",");for(i=0;i<v.length;++i)cols.push("#"+v[i]);}
...
{var v=msg.split("|");if(v[0]=="ShivaAct=resize"){if(v[1]=="100")
shivaLib.options.width=shivaLib.options.height="100%";shivaLib.DrawGraph();}
