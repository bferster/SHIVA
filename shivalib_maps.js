...
SHIVA_Show.prototype.DrawEarthOverlays=function() 					//	DRAW MAP OVERLAYS
{
	...
	for (i=0;i<items.length;++i) {
		...
		if (items[i].layerType == "MarkerSet") {						// MarkerSet layer
			...
			this.GetSpreadsheet(items[i].layerSource,true,null,null,function(d){_this.EarthAddMarkers(d,_this.items[_this.markerData].obj)});
			}
		if (obj) {														// If an object
	...
	for (i=0;i<items.length;++i) {
		...
		if (items[i].layerType == "Drawn") {
			...
			if (items[i].visible == "true") {
				this.items[i].obj=[];
				this.markerData=i;
				this.GetSpreadsheet(items[i].layerSource,true,null,null,function(d){_this.MapAddMarkers(d,_this.items[_this.markerData].obj)});
				}
			continue;
			}
