...
str+="<button type='button' id='"+id+"' onclick='shivaLib.SizingBox(\"containerDiv\",this.id)'>Set box</button>";else if(props[o].opt=="hidden")
...
str+="<textarea cols='13' rows='2' onChange='Draw()' id='"+id2+"' onFocus='ShowHelp(\""+props[oo].des+"\")'/>";else if(props[oo].opt=="hidden")
...
str+="<button type='button' id='"+id2+"' onclick='shivaLib.SizingBox(\"containerDiv\",this.id)'>Set box</button>";else
...
v[1]=v[0],v[0]=0;if(this.smartPlayerType=="Vimeo")
shivaLib.player.currentTime(shivaLib.options.start);else
shivaLib.player.currentTime(Number(v[0]*60)+Number(v[1]));shivaLib.player.volume(shivaLib.options.volume/100);if(shivaLib.options.autoplay=="true")
...
function wordCloud(container){this.d=[];this.filterSet=[];this.container=container;this.draw=function(data){var word=data;$('svg').remove();$('#cloudLoad').remove();d3.select("#"+cloud.container).append("svg").attr("id","wordCloud").attr("width",cloud.options.width).attr("height",cloud.options.height).append("g").attr("transform","translate("+cloud.options.width/2.15+","+((cloud.options.height/2.15))+")").append('rect').attr("transform","translate(-"+cloud.options.width/2.15+",-"+((cloud.options.height/2.15))+")").attr('x',0).attr('y',0).attr('width','100%').attr('height','80%').style('fill',(cloud.options.backgroundColor=="")?'white':cloud.options.backgroundColor);d3.select('g').selectAll("text").data(data).enter().append("text").attr('class','word').style("font-size",function(d){return d.size+"px";}).style("font-family",cloud.options.font_name).style("fill",function(d,i){return fill(i);}).attr("text-anchor","middle").attr("transform",function(d){return"translate("+[d.x+30,d.y]+")rotate("+d.rotate+")";}).text(function(d){return d.text;});d3.select('svg').append('text').attr('id','cloudTitle').text(cloud.options.title).style('font-size',cloud.options.titleFontSize+'px').attr('text-anchor','middle').attr('y',cloud.options.height-35).attr('x',cloud.options.width/2);if($('#wordCloudWordList').length==0){$('#containerDiv').append($('<a>').attr('id','cloudShowListButton').css({position:'absolute',top:'10px',left:0,width:'25px',height:'20px'}).click(function(){$('#wordCloudWordList').toggle();}).button({icons:{primary:'ui-icon-script'}}).hide());$('#containerDiv').append($('<div>',{id:'wordCloudWordList',css:{position:'absolute',top:'10px',left:'20px',height:(cloud.options.height*0.6)+"px",width:'120px',borderRadius:'8px',border:'5px solid #EEE',backgroundColor:'white',overflow:'scroll',padding:'5px',}}).hide());}
d3.selectAll('.listEntry').remove();d3.select('#wordCloudWordList').selectAll('.listEntry').data(data).enter().append('div').attr('class','listEntry').style('vertical-align','middle').style('height','20px').style('width','100px').text(function(d){return d.text+" ("+d.freq+")";}).on('click',function(d){console.log(d.text+" : "+Math.ceil(d.size/((cloud.options.height/3)/cloud.max)));shivaLib.SendShivaMessage("ShivaWord=click|"+d.text+"|"+Math.ceil(d.size/((cloud.options.height/3)/cloud.max)));$('.listEntry').css('backgroundColor','white');$(this).css('backgroundColor','rgba(255,255,105,0.5)');});$('.listEntry').append($('<span>',{css:{float:'right'}}).addClass('listEntryFilter ui-icon ui-icon-close').click(function(e){e.stopPropagation();if($(this).hasClass('ui-icon-close')){$(this).removeClass('ui-icon-close').addClass('ui-icon-arrowreturnthick-1-w');$(this).parent().css('opacity',0.5);}
else{$(this).removeClass('ui-icon-arrowreturnthick-1-w').addClass('ui-icon-close');$(this).parent().css('opacity',1);}
var word=$(this).parent().text().split(' ')[0];cloud.filter();}));if(cloud.options.wordlist=="true"){$('#cloudShowListButton').show();}
...
cloud.d=data;cloud.buildLayout(data);});};this.filter=function(){var words=[];$('.listEntry').filter(function(){return $(this).children('span').hasClass('ui-icon-close');}).each(function(){words.push($(this).text().split(' ')[0]);});var d=cloud.d.filter(function(el){return words.indexOf(el.text)>=0;});cloud.buildLayout(d);};this.colorize=function(colors){var opts=colors.split(',').slice(0,-1);if(opts.length==1)
...
{var v;var options=this.options;var container=this.container;var con="#"+container;var _this=this;if(!options.chartType)
...
this.SendReadyMessage(true);}
else if(options.chartType=="Montage"){$(con).css('height',options.height+"px");$(con).css('width',options.width+"px");this.imageMob={sx:0,ex:1,sy:00,ey:0,sw:100,ew:100,sa:1,ea:1,easeIn:1,easeOut:1,dur:4,div:this.container+"Img",start:0};var o=items[0];v=o.sp.split(";");this.imageMob.sx=v[0]-0;this.imageMob.sy=v[1]-0;this.imageMob.sw=v[2]-0;v=o.ep.split(";");this.imageMob.ex=v[0]-0;this.imageMob.ey=v[1]-0;this.imageMob.ew=v[2]-0;this.imageMob.dur=o.dur;this.imageMob.title=o.title;this.imageMob.fx=o.fx;this.imageMob.url=o.url;$(con).html("<img id='"+this.container+"Img' "+"' src='"+o.url+"'/>");this.AnimateDiv();}
...
SHIVA_Show.prototype.AnimateDiv=function()
{var mob=shivaLib.imageMob;$("#"+shivaLib.container).css("overflow","hidden");if(($("#"+shivaLib.container+"PlyBut").length==0)&&mob.dur){$("#"+shivaLib.container).append("<img id='"+this.container+"PlyBut' src='playbut.gif' style='position:absolute;top:48%;left:47%;padding:2px;padding-left:18px;padding-right:18px' class='propTable' width='18'>");$("#"+shivaLib.container+"PlyBut").click(function(){$(this).hide();shivaLib.imageMob.start=new Date().getTime();shivaLib.imageMob.interval=setInterval(shivaLib.AnimateDiv,42);});}
var pct=(new Date().getTime()-mob.start)/(mob.dur*1000);if(mob.start==0)
pct=0;if(pct>=.99){$("#"+shivaLib.container+"PlyBut").show();clearInterval(shivaLib.imageMob.interval);}
if(mob.easeIn&&mob.easeOut)
