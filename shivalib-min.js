...
stimeline.events=null;stimeline.options=this.options;stimeline.container=this.container;stimeline.con="#"+stimeline.container;if($(stimeline.con).find('*').length>0){var ret=$(stimeline.con).timeline('setOptions',jQuery.extend(true,{},stimeline.options),true);if(ret){$(stimeline.con).timeline('resize');}}
...
shivaLib.SendReadyMessage(true);}});}else{var callbackObj={fn:function(args,data){$(stimeline.con).timeline('setOptions',stimeline.options,true);$(stimeline.con).timeline('registerEvents',stimeline.events);setTimeout('$(\''+stimeline.con+'\').timeline(\'eventList\')',500);if(stimeline.options.show_desc=="false"){$('.tg-timeline-modal').fadeOut();}},args:{"min_zoom":stimeline.options.min_zoom*1,"max_zoom":stimeline.options.max_zoom*1,"show_footer":Boolean(stimeline.options.show_footer),"display_zoom_level":Boolean(stimeline.options.display_zoom_level)},display:true};$(stimeline.con).timeline('loadTimeline',stldata,callbackObj);}
window.stlInterval=setInterval(function(){$('.timeglider-ev-modal').draggable({cancel:'div.tg-ev-modal-description'});},500);function ConvertTimelineDate(dateTime){dateTime=Date.parse(dateTime)+50000000;var sign=(dateTime<0)?-1:1;var dt=new Date(dateTime);dt.setFullYear(dt.getFullYear()*sign);var mn=padZero(dt.getMonth()+1);var dy=padZero(dt.getDate());var hrs=padZero(dt.getHours());var mns=padZero(dt.getMinutes());var scs=padZero(dt.getSeconds());var dtstr=dt.getFullYear()+"-"+mn+"-"+dy+" "+hrs+":"+mns+":"+scs;return dtstr;}
...
if(typeof(ev.enddate)=="undefined"||ev.enddate==""){ev.enddate=ev.startdate;}
