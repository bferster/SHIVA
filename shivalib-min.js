...
shivaLib.player.pause();$("#shivaEventDiv").height(Math.max(shivaLib.player.media.clientHeight-40,0));shivaLib.VideoNotes();shivaLib.SendShivaMessage("ShivaVideo=ready|"+window.name);}
...
d3.selectAll('.listEntry').remove();d3.select('#wordCloudWordList').selectAll('.listEntry').data(wcloud.d).enter().append('div').attr('class','listEntry').style('vertical-align','middle').style('height','20px').style('width','100px').text(function(d){return d.text+" ("+d.freq+")";}).on('click',function(d){console.log(d.text+" : "+d.freq);shivaLib.SendShivaMessage("ShivaWord=click|"+window.name+"|"+d.text+"|"+d.freq);$('.listEntry').css('backgroundColor','white');$(this).css('backgroundColor','rgba(255,255,105,0.5)');});var listClickHandler=function(e,pass){e.preventDefault();e.stopPropagation();if(typeof pass=="undefined")
...
wcloud.colorize(wcloud.options.spectrum);d3.selectAll('.word').on("click",function(d){console.log(d.text+" : "+d.freq);shivaLib.SendShivaMessage("ShivaWord=click|"+window.name+"|"+d.text+"|"+d.freq);});if(!wcloud.ready){shivaLib.SendShivaMessage("ShivaWord=ready|"+window.name);wcloud.ready=true;}};this.buildLayout=function(words){if(typeof wcloud.options.height=="string"&&wcloud.options.height.indexOf('%')!=-1)
...
function EvA()
{this.ondos=new Array();this.data=new Object();if(window.addEventListener)
...
run[o]=ondo[o];if(!run.p1){var v=data.split("|");if(v[1]!=undefined)run.p1=v[1];if(v[2]!=undefined)run.p2=v[2];if(v[3]!=undefined)run.p3=v[3];if(v[4]!=undefined)run.p4=v[4];if(v[5]!=undefined)run.p5=v[5];if(v[6]!=undefined)run.p6=v[6];}
if(ondo.script)
run=window[ondo.script](run);this.RunOnDo(run);}
EvA.prototype.SendMessage=function(con,msg)
{var win=document.getElementById(con).contentWindow;win.postMessage(msg,"*");}
EvA.prototype.LoadSpreadsheet=function(ondo)
{var base="https://docs.google.com/spreadsheet/ccc?key=";var query=new google.visualization.Query(base+ondo.src);query.send(handleQueryResponse);var _this=this;function handleQueryResponse(response){var i,j,lab,n;var o=[],oo=[];var data=response.getDataTable();var cols=data.getNumberOfColumns();var rows=data.getNumberOfRows();for(i=0;i<cols;++i){lab=$.trim(data.getColumnLabel(i));if(!lab)
break;o.push($.trim(lab));}
n=o.length;oo.push(o);for(i=0;i<rows;++i){o=[];for(j=0;j<n;++j)
o.push(data.getValue(i,j));oo.push(o);}
_this.data[ondo.id]=oo;}}
