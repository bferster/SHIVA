...
DrawPosterOverviewGrid();var css={position:"absolute",width:w/this.posterScale+"px",height:h/this.posterScale+"px",border:"1px solid #666","z-index":3,"background-color":"rgba(255,255,255,0.4)"};str+="<div id='posterOverBox'></div>";$("#posterOverDiv").append(str);$("#posterOverBox").css(css);var l=$("#posterOverDiv").offset().left;var t=$("#posterOverDiv").offset().top;var r=(l+w)-(w/this.posterScale)
...
{var i,v,str,dw,dh,x,y,s=0,isImg=true;var scale=this.posterScale;var e=this.items.length;var w=$("#posterDiv").width();var h=$("#posterDiv").height();if(num!=undefined)s=num,e=num+1;for(i=0;i<e;++i){v=this.items[i].data.split("|");dw=v[0]/1000*w;dh=v[0]/1000*h;x=w*v[1]/1000-(dw/2);y=h*v[2]/1000-(dh/2);str="<div id='posterPane"+i+"' class='propTable' style='position:absolute;padding:4px;";str+="left:"+x+"px;";str+="top:"+y+"px;";str+="height:"+dh+" px;";str+="width:"+dw+"px'>";if(isImg=items[i].url.match(/[[.]jpg|jpeg|gif|png]/i))
str+="<img src='"+items[i].url+"' width='"+dw+"'>";else
str+="<iframe src='"+items[i].url+"' height='"+dh+"' width='"+dw+"' frameborder='0' ALLOWTRANSPARENCY='true'></iframe>";str+="<br/><span><b>"+(i+1)+". "+items[i].layerTitle+"</b></span>"
$("#posterDiv").append(str+"</div>");$("#posterPane"+i).resizable({containment:"parent",aspectRatio:isImg,stop:function(event,ui){var i=event.target.id.substr(10);var v=items[i].data.split("|");v[0]=Math.floor(Math.min(ui.size.width/$("#containerDiv").width()/shivaLib.posterScale,1)*1000);items[i].data=v[0]+"|"+v[1]+"|"+v[2];$("#itemInput"+i+"-1").val(items[i].data);Draw();}});$("#posterPane"+i).draggable({containment:"parent",stop:function(event,ui){var i=event.target.id.substr(10);var v=items[i].data.split("|");var w=$("#posterDiv").width();var h=$("#posterDiv").height();var off=-7;if(shivaLib.items[i].url.match(/[[.]jpg|jpeg|gif|png]/i))
off=15;v[1]=Math.round(($("#posterPane"+i).position().left+$("#posterPane"+i).width()/2)/w*1000);v[2]=Math.round(($("#posterPane"+i).position().top+$("#posterPane"+i).height()/2+off)/h*1000);items[i].data=v[0]+"|"+v[1]+"|"+v[2];$("#itemInput"+i+"-1").val(items[i].data);Draw();}});if(this.options.overview=="true"){str="<div style='position:absolute;opacity:.4;border:1px solid;pointer-events:none;background-color:#666;";str+="left:"+x/4/scale+"px;";str+="top:"+y/scale/4+"px;";str+="height:"+dh/4/scale+"px;";str+="width:"+dw/4/scale+"px'></div>";$("#posterOverDiv").append(str);}}}
