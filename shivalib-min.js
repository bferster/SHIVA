...
return this[i];}
return false;}
var cloud;var fill=d3.scale.category20();if(!this.wcloud){cloud=new wordCloud(this.container);this.wcloud=cloud;cloud.options=this.options;cloud.load(this.options.dataSourceUrl);}else{cloud=this.wcloud;var props=Object.keys(this.options);for(var i=0;i<props.length;i++){var prop=props[i];if(this.wcloud.options[prop]!=this.options[prop]){if(prop=="dataSourceUrl"){cloud.options=this.options;cloud.load(cloud.options['dataSourceUrl']);break;}else if(prop=="width"||prop=="height"||prop=="low_threshold"||prop=="high_threshold"||prop=='tiltRange'||prop=="scale"){cloud.options=this.options;cloud.buildLayout(cloud.d);}else{switch(prop){case'font_name':d3.selectAll('text').style('font-family',this.options.font_name);break;case'backgroundColor':d3.select('rect').style('fill','#'+this.options.backgroundColor);break;case'spectrum':cloud.colorize(this.options.spectrum);break;case'title':d3.select('#cloudTitle').text(this.options.title);break;case'titleColor':d3.select('#cloudTitle').attr('fill','#'+this.options.titleColor);break;case'titleFontSize':d3.select('#cloudTitle').style('font-size',this.options.titleFontSize+'px');break;case'wordList':if(this.options.wordList=="true")
$('#cloudShowListButton').show();else{$('#cloudShowListButton').hide();$('#wordCloudWordList').hide();}
break;}}}}
cloud.options=this.options;}
function wordCloud(container){this.d=[];this.filterSet=[];this.container=container;this.draw=function(data){var word=data;$('svg').remove();$('#cloudLoad').remove();d3.select("#"+cloud.container).append("svg").attr("id","wordCloud").attr("width",cloud.options.width).attr("height",cloud.options.height).append("g").attr("transform","translate("+((cloud.options.width/2))+","+((cloud.options.height/2))+")").append('rect').attr("transform","translate(-"+((cloud.options.width/2))+",-"+((cloud.options.height/2)*0.8)+")").attr('x',0).attr('y',0).attr('width','100%').attr('height','80%').style('fill',(cloud.options.backgroundColor=="")?'white':cloud.options.backgroundColor);d3.select('g').selectAll("text").data(data).enter().append("text").attr('class','word').style("font-size",function(d){return d.size+"px";}).style("font-family",cloud.options.font_name).style("fill",function(d,i){return fill(i);}).attr("text-anchor","middle").attr("transform",function(d){return"translate("+[d.x+30,d.y]+")rotate("+d.rotate+")";}).text(function(d){return d.text;});d3.select('svg').append('text').attr('id','cloudTitle').text(cloud.options.title).style('font-size',cloud.options.titleFontSize+'px').attr('text-anchor','middle').attr('y',cloud.options.height-(cloud.options.titleFontSize)).attr('x',cloud.options.width/2);if($('#wordCloudWordList').length==0){$('#containerDiv').append($('<a>').attr('id','cloudShowListButton').css({position:'absolute',top:'10px',left:0,width:'25px',height:'20px'}).click(function(){$('#wordCloudWordList').toggle();}).button({icons:{primary:'ui-icon-script'}}).hide());$('#containerDiv').append($('<div>',{id:'wordCloudWordList',css:{position:'absolute',top:'10px',left:'20px',height:(cloud.options.height*0.6)+"px",width:'120px',borderRadius:'8px',border:'5px solid #EEE',backgroundColor:'white',overflow:'scroll',padding:'5px',}}).hide());}
d3.selectAll('.listEntry').remove();d3.select('#wordCloudWordList').selectAll('.listEntry').data(cloud.d).enter().append('div').attr('class','listEntry').style('vertical-align','middle').style('height','20px').style('width','100px').text(function(d){return d.text+" ("+d.freq+")";}).on('click',function(d){console.log(d.text+" : "+Math.ceil(d.size/((cloud.options.height/3)/cloud.max)));shivaLib.SendShivaMessage("ShivaWord=click|"+d.text+"|"+Math.ceil(d.size/((cloud.options.height/3)/cloud.max)));$('.listEntry').css('backgroundColor','white');$(this).css('backgroundColor','rgba(255,255,105,0.5)');});$('.listEntry').append($('<span>',{css:{float:'right'}}).addClass('listEntryFilter ui-icon ui-icon-close').on('click',function(e){var pass;if(typeof e.data=="undefined")
pass=false;else{if(typeof e.data.pass=="undefined")
pass=false;else{pass=e.data.pass;}}
e.stopPropagation();if($(this).hasClass('ui-icon-close')){$(this).removeClass('ui-icon-close').addClass('ui-icon-arrowreturnthick-1-w');$(this).parent().css('opacity',0.5);}
...
cloud.filter();}));$('.listEntry').each(function(){if(!data.find($(this).text().split(' ')[0],"text"))
$(this).find('span').click({pass:true})});if(cloud.options.wordlist=="true"){$('#cloudShowListButton').show();}
if(cloud.options.spectrum!="")
cloud.colorize(cloud.options.spectrum);d3.selectAll('.word').on("click",function(d){console.log(d.text+" : "+Math.ceil(d.size/((cloud.options.height/3)/cloud.max)));shivaLib.SendShivaMessage("ShivaWord=click|"+d.text+"|"+Math.ceil(d.size/((cloud.options.height/3)/cloud.max)));});shivaLib.SendShivaMessage("ShivaWord=ready");};this.buildLayout=function(data){var fs;if(cloud.options.scale=="logarithmic")
...
cloud.d=data;cloud.buildLayout(data);});};this.filter=function(){var words=[];$('.listEntry').filter(function(){return $(this).children('span').hasClass('ui-icon-close');}).each(function(){words.push($(this).text().split(' ')[0]);});var d=cloud.d.filter(function(el){if(words.indexOf(el.text)==-1){cloud.filterSet.push(el.text);return false;}
else
...
this.posterX=this.posterY=1/this.posterScale/2;$(con).html(str);$(con).css({"width":options.width+"px","height":options.height+"px"});$(con).css({border:"1px solid",overflow:"hidden",margin:"0px",padding:"0px"});$("#posterDiv").width(options.width*this.posterScale);$("#posterDiv").height(options.height*this.posterScale);$("#posterDiv").css("background-color","#"+options.backCol);var l=$(con).css("left").replace(/px/,"")-0;var r=l-0+(options.width-(options.width*this.posterScale));var t=$(con).css("top").replace(/px/,"")-0;var b=t-0+(options.height-(options.height*this.posterScale));$("#posterDiv").draggable({containment:[r,b,l,t],drag:function(event,ui){var w=$("#posterDiv").width();var h=$("#posterDiv").height();var s=shivaLib.posterScale;shivaLib.posterX=(-$("#posterDiv").css("left").replace(/px/,"")+(w/s/2))/w;shivaLib.posterY=(-$("#posterDiv").css("top").replace(/px/,"")+(h/s/2))/h;shivaLib.DrawPosterOverview();}});if(options.dataSourceUrl){str="<img src='"+options.dataSourceUrl+"' ";str+="height='"+options.height*this.posterScale+"' ";str+="width='"+options.width*this.posterScale+"'>";$("#posterDiv").append(str);}
...
DrawPosterGrid();this.DrawPosterOverview();if(mode!="Image")
this.DrawPosterPanes();this.DrawLayerControlBox(this.items,(options.controlbox=="true"));this.SendReadyMessage(true);}
...
DrawPosterOverviewGrid();var css={position:"absolute",width:w/this.posterScale+"px",height:h/this.posterScale+"px",border:"1px solid #666","z-index":3,"background-color":"rgba(220,220,220,0.4)"};str+="<div id='posterOverBox'></div>";$("#posterOverDiv").append(str);$("#posterOverBox").css(css);var l=$("#posterOverDiv").offset().left;var t=$("#posterOverDiv").offset().top;var r=(l+w)-(w/this.posterScale)
var b=(t+h)-(h/this.posterScale)
$("#posterOverBox").draggable({containment:[l,t,r,b],drag:function(event,ui){var w=$("#posterOverDiv").width();var pw=$("#posterDiv").width();var h=$("#posterOverDiv").height();var ph=$("#posterDiv").height();var s=shivaLib.posterScale;var x=Math.max(0,ui.position.left/w*pw);var y=Math.max(0,ui.position.top/h*ph);shivaLib.posterX=(x+(pw/s/2))/pw;shivaLib.posterY=(y+(ph/s/2))/ph;$("#posterDiv").css({"left":-x+"px","top":-y+"px"});}});}
$("#posterOverBox").resizable({containment:"parent",aspectRatio:true,minHeight:12,stop:function(event,ui){var w=$("#posterOverDiv").width();var pw=$("#posterDiv").width();var h=$("#posterOverDiv").height();var ph=$("#posterDiv").height();shivaLib.posterScale=Math.max(w/ui.size.width,1);var s=shivaLib.posterScale;var x=Math.max(0,ui.position.left/w*pw);var y=Math.max(0,ui.position.top/h*ph);shivaLib.posterX=(x+(pw/s/2))/pw;shivaLib.posterY=(y+(ph/s/2))/ph;shivaLib.DrawPoster();}});var x=options.left=$("#posterDiv").css("left").replace(/px/,"");x=-x/options.width*w/this.posterScale;var y=options.top=$("#posterDiv").css("top").replace(/px/,"");y=-y/options.height*h/this.posterScale;$("#posterOverBox").css({"left":x+"px","top":y+"px"});}
SHIVA_Show.prototype.DrawPosterPanes=function(num)
{var i,v,str,dw,dh,x,y,s=0,isImg=true;var scale=this.posterScale;var e=this.items.length;var w=$("#posterDiv").width();var h=$("#posterDiv").height();if(num!=undefined)s=num,e=num-0+1;for(i=0;i<e;++i){trace(s,e)
v=this.items[i].data.split("|");dw=v[0]/1000*w;dh=v[0]/1000*h;x=w*v[1]/1000-(dw/2);y=h*v[2]/1000-(dh/2);str="<div id='posterPane"+i+"' style='position:absolute;background:none transparent;";str+="left:"+x+"px;";str+="top:"+y+"px;";str+="height:"+dh+" px;";str+="width:"+dw+"px'>";if(isImg=this.items[i].url.match(/[[.]jpg|jpeg|gif|png]/i))
str+="<img src='"+this.items[i].url+"' width='"+dw+"'>";else
str+="<iframe src='"+this.items[i].url+"' height='"+dh+"' width='"+dw+"' scrolling='no' frameborder='0' allowtransparency='true'></iframe>";if(this.posterMode=="Edit"){str+="<br/><div style='height:18px' class='propTable'>";str+="<span style='vertical-align:middle'><b>&nbsp; "+(i+1)+". "+this.items[i].layerTitle+"</b></span></div>";}
if(num==undefined)
$("#posterDiv").append(str+"</div>");if(this.options.overview=="true"){str="<div id='posterOverPane"+i+"' style='position:absolute;opacity:.4;border:1px solid;pointer-events:none;background-color:#666;";str+="height:"+dh/4/scale+"px;";str+="width:"+dw/4/scale+"px'></div>";if(num==undefined)
$("#posterOverDiv").append(str);x=$("#posterPane"+i).position().left;y=$("#posterPane"+i).position().top;$("#posterOverPane"+i).css({"left":x/4/scale+"px","top":y/4/scale+"px"});}
if(this.posterMode!="Edit")
continue;$("#posterPane"+i).resizable({containment:"parent",aspectRatio:isImg,stop:function(event,ui){var i=event.target.id.substr(10);var v=shivaLib.items[i].data.split("|");v[0]=Math.floor(Math.min(ui.size.width/$("#containerDiv").width()/shivaLib.posterScale,1)*1000);shivaLib.items[i].data=v[0]+"|"+v[1]+"|"+v[2];$("#itemInput"+i+"-1").val(shivaLib.items[i].data);Draw();}});$("#posterPane"+i).draggable({containment:"parent",drag:function(event,ui){var i=event.target.id.substr(10);var v=items[i].data.split("|");var w=$("#posterDiv").width();var h=$("#posterDiv").height();var off=-11;if(shivaLib.items[i].url.match(/[[.]jpg|jpeg|gif|png]/i))
off=15;v[1]=Math.round(($("#posterPane"+i).position().left+$("#posterPane"+i).width()/2)/w*1000);v[2]=Math.round(($("#posterPane"+i).position().top+$("#posterPane"+i).height()/2+off)/h*1000);shivaLib.items[i].data=v[0]+"|"+v[1]+"|"+v[2];$("#itemInput"+i+"-1").val(shivaLib.items[i].data);shivaLib.DrawPosterPanes(i);}});}}
