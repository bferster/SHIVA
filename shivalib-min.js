...
d3.selectAll('.listEntry').remove();d3.select('#wordCloudWordList').selectAll('.listEntry').data(cloud.d).enter().append('div').attr('class','listEntry').style('vertical-align','middle').style('height','20px').style('width','100px').text(function(d){return d.text+" ("+d.freq+")";}).on('click',function(d){console.log(d.text+" : "+d.freq);shivaLib.SendShivaMessage("ShivaWord=click|"+window.name+"|"+d.text+"|"+d.freq);$('.listEntry').css('backgroundColor','white');$(this).css('backgroundColor','rgba(255,255,105,0.5)');});var listClickHandler=function(e,pass){if(typeof pass=="undefined")
pass=false;e.stopPropagation();if($(this).hasClass('ui-icon-close')){$(this).removeClass('ui-icon-close').addClass('ui-icon-arrowreturnthick-1-w');$(this).parent().css('opacity',0.5);}
...
cloud.filter();};$('.listEntry').append($('<span>',{css:{float:'right'}}).addClass('listEntryFilter ui-icon ui-icon-close').on('click',listClickHandler));$('.listEntry').each(function(){if(!data.find($(this).text().split(' ')[0],"text"))
$(this).find('span').trigger('click',[true]);});if(cloud.options.wordlist=="true"){$('#cloudShowListButton').show();}
...
else{this.wcloud.options.width=cmd[1];this.wcloud.options.height=cmd[1];}
this.wcloud.buildLayout(wcloud.d);break;}}
