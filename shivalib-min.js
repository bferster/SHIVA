...
{var options=this.options;var container=this.container;var con="#"+container;var h=$(con).css('height');var w=$(con).css('width');var _this=this;this.imageMob={sx:0,ex:1,sy:00,ey:0,sw:200,ew:100,sa:1,ea:1,easeIn:1,easeOut:1,dur:4000,div:this.container+"Img",start:0};if($("#propInput6").val()=="666")options.dataSourceUrl="http://www.primaryaccess.org/test.jpg";if(options.dataSourceUrl.indexOf("//docs.google.com")!=-1)
...
$(con).css('height',options.height);trace($("#propInput6").val())
if($("#propInput6").val()=="666")
this.AnimateDiv();this.SendReadyMessage(true);}
else
...
{var mob=shivaLib.imageMob;$("#"+shivaLib.container).css("overflow","hidden");if(($("#"+shivaLib.container+"PlyBut").length==0)&&mob.dur){$("#"+shivaLib.container).append("<img id='"+this.container+"PlyBut' src='playbut.gif' style='position:absolute;top:50%;left:50%;padding:2px;padding-left:18px;padding-right:18px' class='propTable' width='18'>");$("#"+shivaLib.container+"PlyBut").click(function(){$(this).hide();shivaLib.imageMob.start=new Date().getTime();shivaLib.imageMob.interval=setInterval(shivaLib.AnimateDiv,42);});}
var pct=(new Date().getTime()-mob.start)/mob.dur;if(mob.start==0)
pct=0;if(pct>=1){$("#"+shivaLib.container+"PlyBut").show();clearInterval(shivaLib.imageMob.interval);}
if(mob.easeIn&&mob.easeOut)
pct=1.0-((Math.cos(3.1414*pct)+1)/2.0);else if(easeIn)
pct=1.0-(Math.cos(1.5707*pct));else if(easeOut)
pct=1.0-(Math.cos(1.5707+(1.5707*pct))+1.0);var o={position:"relative"};o.left=-(mob.sx+((mob.ex-mob.sx)*pct))+"%";o.top=-(mob.sy+((mob.ey-mob.sy)*pct))+"%";o.width=(mob.sw+((mob.ew-mob.sw)*pct)+"%");o.opacity=mob.sa+((mob.ea-mob.sa)*pct);$("#"+mob.div).css(o);}
...
function wordCloud(container){this.d=[];this.filterSet=[];this.container=container;this.draw=function(words){$('#containerDiv').html('');d3.select("#"+cloud.container).append("svg").attr("id","wordCloud").attr("width",cloud.options.width).attr("height",cloud.options.height).append("g").attr("transform","translate("+cloud.options.width/2.15+","+((cloud.options.height/2.15))+")").append('rect').attr("transform","translate(-"+cloud.options.width/2.15+",-"+((cloud.options.height/2.15))+")").attr('x',0).attr('y',0).attr('width','100%').attr('height','80%').style('fill',(cloud.options.backgroundColor=="")?'white':cloud.options.backgroundColor);d3.select('g').selectAll("text").data(words).enter().append("text").attr('class','word').style("font-size",function(d){return d.size+"px";}).style("font-family",cloud.options.font_name).style("fill",function(d,i){return fill(i);}).attr("text-anchor","middle").attr("transform",function(d){return"translate("+[d.x+30,d.y]+")rotate("+d.rotate+")";}).text(function(d){return d.text;});d3.select('svg').append('text').attr('id','cloudTitle').text(cloud.options.title).style('font-size',cloud.options.titleFontSize+'px').attr('text-anchor','middle').attr('y',cloud.options.height-35).attr('x',cloud.options.width/2);d3.selectAll('.word').on("click",function(){console.log(d.text+" : "+d.size);shivaLib.SendShivaMessage("ShivaWord=click|"+d.text+"|"+d.size);});shivaLib.SendShivaMessage("ShivaWord=ready");};this.buildLayout=function(data,max){d3.layout.cloud().size([cloud.options.width,cloud.options.height*0.8]).words(data).rotate(function(){return~~(Math.random()*2)*90;}).font(cloud.options.font_name).fontSize(function(d){return d.size*((cloud.options.height/3)/max);}).on("end",cloud.draw).start();};this.load=function(src){d3.select('svg').remove();var qs='parser.php?'+encodeURIComponent('url')+'='+encodeURIComponent(src);d3.json(qs,function(error,data){if(data.error){if(data.error=="fetch_fail")
