...
SHIVA_Show.prototype.DrawWordCloud=function(){var wcloud;var fill=d3.scale.category20();if(!this.wcloud){wcloud=new wordCloud(this.container);this.wcloud=wcloud;wcloud.options=this.options;wcloud.load(this.options.dataSourceUrl);}else{wcloud=this.wcloud;var props=Object.keys(this.options);for(var i=0;i<props.length;i++){var prop=props[i];if(this.wcloud.options[prop]!=this.options[prop]){if(prop=="dataSourceUrl"){wcloud.options=this.options;wcloud.load(wcloud.options['dataSourceUrl']);break;}else if(prop=="width"||prop=="height"||prop=="low_threshold"||prop=="high_threshold"||prop=='tiltRange'||prop=="scale"){wcloud.options=this.options;wcloud.buildLayout(wcloud.d);}else{switch(prop){case'font_name':d3.selectAll('text').style('font-family',this.options.font_name);break;case'backgroundColor':if(typeof this.options.backgroundColor=='undefined'||this.options.backgroundColor=='')
...
wcloud.colorize(wcloud.options.spectrum);d3.selectAll('.word').on("click",function(d){console.log(d.text+" : "+d.freq);shivaLib.SendShivaMessage("ShivaWord=click|"+window.name+"|"+d.text+"|"+d.freq);});shivaLib.SendShivaMessage("ShivaWord=ready|"+window.name);};this.buildLayout=function(words){if(wcloud.options.height.indexOf('%')!=-1)
wcloud.options.height=$('#containerDiv').height()*(wcloud.options.height.slice(0,-1)/100);if(wcloud.options.width.indexOf('%')!=-1)
...
fs=d3.scale.linear().domain([0,words[0].freq]).range([5,distro*(0.15*wcloud.options.height)]);else if(wcloud.options.scale=="binary")
