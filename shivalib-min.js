...
dat["type"]=type;str="http://www.viseyes.org/shiva/listeasyfile.php";shivaLib.ezcb=callback;shivaLib.ezmode="load";$.ajax({url:str,data:dat,dataType:'jsonp'});});$("#linkBut").button().click(function(){email=$("#email").val();if(!email){alert("Please type your email");return;}
document.cookie="ez-email="+email;var dat={email:email};if(type!="all")
dat["type"]=type;str="http://www.viseyes.org/shiva/listeasyfile.php";shivaLib.ezcb="";shivaLib.ezmode="link";$.ajax({url:str,data:dat,dataType:'jsonp'});});}
...
prompt("Press Ctrl-C to copy link","http://www.viseyes.org/shiva/getkml.php?id="+this.id.substr(7));if((mode=="link")&&(type!="KML"))
prompt("Press Ctrl-C to copy link","www.viseyes.org/shiva/go.htm?e="+this.id.substr(7));else{var dat={id:this.id.substr(7)};str="http://www.viseyes.org/shiva/geteasyfile.php";shivaLib.ezcb=callback;shivaLib.ezmode=this.id.substr(7);$.ajax({url:str,data:dat,dataType:'jsonp'});}
...
{var str="http://www.viseyes.org/shiva/geteasyfile.php";shivaLib.ezcb=callback;shivaLib.ezmode=num;$.ajax({url:str,data:{id:num},dataType:'jsonp'});$("#shivaLightBoxDiv").remove();if($("#containerDiv").height()<10)
$("#containerDiv").height($("#containerDiv").height()*100);}
