...
this.player.on("timeupdate",drawOverlay);this.player.on("loadeddata",onVidLoaded);this.player.on("ended",function(){shivaLib.SendShivaMessage("ShivaPlayer=done")});this.player.on("playing",function(){shivaLib.SendShivaMessage("ShivaPlayer=play")});this.player.on("pause",function(){shivaLib.SendShivaMessage("ShivaPlayer=pause")});if(this.ev)
...
shivaLib.player.pause();$("#shivaEventDiv").height(Math.max(shivaLib.player.media.clientHeight-40,0));shivaLib.SendShivaMessage("ShivaPlayer=ready");}
...
stimeline.events=null;stimeline.options=this.options;stimeline.container=this.container;stimeline.con="#"+stimeline.container;if($(stimeline.con).find('*').length>0){var ret=$(stimeline.con).timeline('setOptions',jQuery.extend(true,{},stimeline.options),false);if(ret){$(stimeline.con).timeline('resize');return;}}
$(stimeline.con).css('width',stimeline.options['width']+"px");$(stimeline.con).css('height',stimeline.options['height']+"px");GetSpreadsheetData(stimeline.options.dataSourceUrl);function GetSpreadsheetData(file,conditions)
...
if(typeof(ev.importance)=="undefined"){ev.importance=50;}
if(typeof(ev.date_display)=="undefined"){ev.date_display="ye";}
if(typeof(ev.icon)=="undefined"){ev.icon="none";}}
return events;}}}}
