...
SHIVA_Show.prototype.EasyFile=function(_data, callback, type) 			// EASYFILE MENU
{
	...
	$("#saveBut").button().click(function() {								// SAVE
		...
		$.post("http://www.stagetools.com/shiva/addeasyfile.php",{ email:_email, type: _type, title:_title,data:_data.replace(/'/g,"\\'") });
		});
	...
	$("#loadBut").button().click(function() {								// LOAD
		...
		str="http://www.stagetools.com/shiva/listeasyfile.php";				// eStore list url
	...
	for (i=0;i<files.length;++i) {											// For each file
		...
		$("#ezfile-"+files[i].id).click(function() {						// Add click handler
			if ((mode == "link") && (type == "KML"))						// If a KML link
				prompt("Press Ctrl-C to copy link","http://www.stagetools.com/shiva/getkml.php?id="+this.id.substr(7));	// Show url
			if ((mode == "link") && (type != "KML"))						// If a SHIVA link
				...
				str="http://www.stagetools.com/shiva/geteasyfile.php";	// eStore list url
...
{
	var str="http://www.stagetools.com/shiva/geteasyfile.php";			// eStore url
	shivaLib.ezcb=callback;													// Set callback
