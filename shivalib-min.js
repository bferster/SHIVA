...
_data=_data.substr(0,_data.lastIndexOf("\""))+str;$.post("http://www.viseyes.org/shiva/addeasyfile.php",{email:_email,type:_type,title:_title,data:_data.replace(/'/g,"\\'")});});$("#loadBut").button().click(function(){email=$("#email").val();if(!email){alert("Please type your email");return;}
if(!isNaN(email)){shivaLib.LoadEasyFile(email,callback);return;}
document.cookie="ez-email="+email;var dat={email:email};if(type!="all")
dat["type"]=type;str="http://www.viseyes.org/shiva/shiva/listeasyfile.php";shivaLib.ezcb=callback;shivaLib.ezmode="load";$.ajax({url:str,data:dat,dataType:'jsonp'});});$("#linkBut").button().click(function(){email=$("#email").val();if(!email){alert("Please type your email");return;}
document.cookie="ez-email="+email;var dat={email:email};if(type!="all")
dat["type"]=type;str="http://www.viseyes.org/shiva/shiva/listeasyfile.php";shivaLib.ezcb="";shivaLib.ezmode="link";$.ajax({url:str,data:dat,dataType:'jsonp'});});}
...
prompt("Press Ctrl-C to copy link","http://www.viseyes.org/shiva/shiva/getkml.php?id="+this.id.substr(7));if((mode=="link")&&(type!="KML"))
prompt("Press Ctrl-C to copy link","www.viseyes.org/shiva/go.htm?e="+this.id.substr(7));else{var dat={id:this.id.substr(7)};str="http://www.viseyes.org/shiva/shiva/geteasyfile.php";shivaLib.ezcb=callback;shivaLib.ezmode=this.id.substr(7);$.ajax({url:str,data:dat,dataType:'jsonp'});}
...
{var str="http://www.viseyes.org/shiva/shiva/geteasyfile.php";shivaLib.ezcb=callback;shivaLib.ezmode=num;$.ajax({url:str,data:{id:num},dataType:'jsonp'});$("#shivaLightBoxDiv").remove();if($("#containerDiv").height()<10)
$("#containerDiv").height($("#containerDiv").height()*100);}
