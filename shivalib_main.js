...
SHIVA_Show.prototype.EasyFile=function(_data, callback, type) 			// EASYFILE MENU
{
	...
	$("#loadBut").button().click(function() {								// LOAD
		...
		str="http://www.viseyes.org/shiva/shiva/listeasyfile.php";			// eStore list url
	...
	for (i=0;i<files.length;++i) {											// For each file
		...
		$("#ezfile-"+files[i].id).click(function() {						// Add click handler
			if ((mode == "link") && (type == "KML"))						// If a KML link
				prompt("Press Ctrl-C to copy link","http://www.viseyes.org/shiva/shiva/getkml.php?id="+this.id.substr(7));	// Show url
			if ((mode == "link") && (type != "KML"))						// If a SHIVA link
				...
				str="http://www.viseyes.org/shiva/shiva/geteasyfile.php";	// eStore list url
...
{
	var str="http://www.viseyes.org/shiva/shiva/geteasyfile.php";			// eStore url
	shivaLib.ezcb=callback;													// Set callback
